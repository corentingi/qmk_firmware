...
bool MCP_begin() {
    return MCP_byteWrite(IOCON, ADDR_ENABLE);
}

// GENERIC BYTE WRITE - will write a byte to a register, arguments are register address and the value to write

bool MCP_byteWrite(uint8_t reg, uint8_t value) {      // Accept the register and byte
...
bool MCP_pinMode(uint8_t pin, uint8_t mode) {     // Accept the pin # and I/O mode
    if ((pin < 1) | (pin > 16)) return false;     // If the pin value is not valid (1-16) return, do nothing and return
...
bool MCP_pullupMode(uint8_t pin, uint8_t mode) {
    ...
    return MCP_wordWrite(GPPUA, MCP_pullupCache);
}
...
bool MCP_inputInvert(uint8_t pin, uint8_t mode) {
    if ((pin < 1) | (pin > 16)) return false;
    if (mode == ON) {
...
bool MCP_digitalWrite(uint8_t pin, uint8_t value) {
    if ((pin < 1) | (pin > 16)) return false;
    if (value) {
